...
사용자가 강의를 선택하면 상위 학습목표 목록을 조회한다.

`GET /api/learning-objectives?lecture_id=basic_statistics`
...
```json
{
  ...
  "objectives": [
    {
      "learning_objective_id": "PostgreSQL 상위 학습목표 UUID",
      "objective_id": "stats.probability_foundations",
      "title": "확률·통계의 기초",
      "description": "...",
      "display_order": 1
...
프론트는 사용자가 상위 학습목표를 선택하면 응답의 `learning_objective_id`와
`objective_id`를 상태에 보관한다.

### 6.1 복습 세션 생성

로그인 후, 녹음 전에 반드시 복습 세션을 생성한다.
...
- 요청에는 로그인 쿠키가 필요하다.
- 응답의 `id`는 PostgreSQL의 `study_sessions.id` UUID다.
- 이 UUID를 이후 STT 업로드와 평가 요청의 `session_id`로 그대로 사용한다.
- `learning_objective_id`와 RAG 평가용 `objective_id`는 다른 값이다.
  - `learning_objective_id`: DB 상위 학습목표 UUID
  - `objective_id`: 예) `stats.probability_foundations`
...
| 필드 | 타입 | 설명 |
| --- | --- | --- |
| `session_id` | UUID string | `POST /api/study-sessions` 응답의 `id` |
  ...
  "segments": [
    {
      "segment_id": "seg_01",
      "index": 1,
      "text": "의미 단위로 분리된 사용자 발화"
    }
  ],
  "claims": [
    {
      "claim_id": "stats.random_variable",
      "judgment": "partial",
      "source_chunk_ids_used": ["basic_statistics_p4_01"],
      "source_chunks": [{"source_chunk_id": "basic_statistics_p4_01", "page": 4}],
      "conflict_status": "none",
      "evidence_spans": [],
      "rationale": "Claim 판정 근거"
    }
  ],
  "quantitative": {
    "scores": {
...
현재 `/api/reviews/submit` 응답의 `claims`에는 Claim별 `judgment`, `conflict_status`,
`evidence_spans`, `rationale`, `source_chunks`가 포함된다. 프론트는 이를 사용해
Claim별 판단, 사용자 발화 근거, 강의자료 페이지 이동 UI를 구현할 수 있다.

`segments`는 보정 전사문을 의미 단위로 나눈 결과다. `evidence_spans`의
`segment_id`를 사용하면 해당 전사 구간을 결과 화면에서 강조할 수 있다.
...
6. 오디오, 전사, 평가 결과는 PostgreSQL의 `audio_files`, `transcriptions`,
   `evaluations`에 저장된다. `evaluations.evaluation_json`에는 Segment와 Claim 상세가
   JSONB로 저장된다.
...
1. 로그인 후 녹음 전에 `POST /api/study-sessions`를 호출하고, 응답 UUID를 보관한다.
2. 강의와 상위 학습목표를 모두 선택하게 한다. DB UUID와 RAG `objective_id`를 구분한다.
3. 녹음 시간은 120초를 기준으로 한다.
4. 녹음 종료 후 `transcribing`, `correcting`, `evaluating` 상태를 구분해 보여준다.
5. `corrected`가 된 뒤에만 `/api/reviews/submit`을 호출한다.
6. 평가 중 중복 제출을 막는다.
7. 결과 화면에 총점뿐 아니라 60/20/20 세부 점수와 이유를 표시한다.
8. `segments`, `claims`를 이용해 Claim별 판단, 근거 발화, 근거 페이지를 표시한다.
9. 누락과 오개념을 서로 다른 영역으로 표시한다.
10. 실패 시 STT 실패와 평가 실패를 구분해 안내한다.
11. 실제 FastAPI 연결 시 `NEXT_PUBLIC_API_BASE_URL`을 반드시 확인한다.
12. Object URL과 MediaStream track을 종료해 브라우저 리소스를 정리한다.
...
2. 로그인 후 `POST /api/study-sessions`가 UUID를 반환하는지 확인
3. 기초통계와 상위 목표 하나 선택
4. 반환 UUID로 WebM 녹음 업로드 후 `202`와 `job_id` 확인
5. Polling에서 `transcribing → correcting → corrected` 확인
6. raw·corrected transcript가 모두 비어 있지 않은지 확인
7. 평가 제출 후 `201`, `status=evaluated` 확인
8. `audio_files`, `transcriptions`, `evaluations`에 동일한 `study_session_id`로 저장됐는지 확인
9. 총점이 0~100이고 세 영역 만점이 60·20·20인지 확인
10. 결과 화면에 누락·오개념·복습 제안 표시
11. 잘못된 파일 형식, topic 불일치, 없는 objective ID 오류 처리 확인
12. 새로고침·서버 재시작 시 현재 메모리 상태 동작 확인

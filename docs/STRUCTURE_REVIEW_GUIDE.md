...
이 결과는 Rubric Evidence의 직접 조회 원본이다. 임베딩 벡터나 ChromaDB는
사용하지 않으며, Claim의 `chunk_id`로 필요한 페이지를 직접 조회한다.
...
- 제목·목차·섹션 구분 페이지도 페이지 누락 방지를 위해 청크로 유지하되 Rubric Evidence에서는 제외한다.
...
그다음 `python scripts/validate_evaluation_data.py`와 `pytest -q`로 페이지 누락,
스키마 오류, 원문·Evidence 불일치를 확인한다. 임베딩 인덱스 생성 단계는 없다.
